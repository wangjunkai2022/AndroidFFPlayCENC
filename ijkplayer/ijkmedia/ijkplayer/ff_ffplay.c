...
int ffp_start_record(FFPlayer *ffp, const char *file_name) {
    ...
    for (int i = 0; i < is->ic->nb_streams; i++) {
        ...
        //            continue;
        //        }
        AVStream *out_stream = avformat_new_stream(ffp->m_ofmt_ctx, avcodec_find_decoder(
        ...
        if (in_stream->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) {
            ...
            if (out_stream->codecpar->width == 0 || out_stream->codecpar->height == 0) {
                if (in_stream->parser) {
                    ...
                    av_log(ffp, AV_LOG_DEBUG, "set width from parser：%d",
                           out_stream->codecpar->width);
                    av_log(ffp, AV_LOG_DEBUG, "set height from parser:%d",
...
//                   pkt->stream_index, out_stream->codec->time_base.den,
//                   out_stream->codec->time_base.num);
